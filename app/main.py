...
EDIT_IN_CATEGORIES_LABEL = "Edit in Categories tab"
...
RECIPE_COURSES = ["Breakfast", "Lunch", "Dinner", "Snack", "Dessert", "Side", "Sauce / Condiment", "Baking", "Drink", "Preserve"]
# Kept for older Pathmark Sync sheets, but no longer shown as a recipe-finder filter.
RECIPE_MEAL_CATEGORIES = []
RECIPE_CUISINE_TAGS = ["Western", "New Zealand", "Italian", "French", "Japanese", "Chinese", "Korean", "Thai", "Vietnamese", "Indian", "Middle Eastern", "Mediterranean", "Mexican", "Fusion"]
RECIPE_DISH_STYLE_TAGS = ["Zenzai", "Agemono", "Nimono", "Donburi", "Nabe", "Mezze", "Gratin", "Risotto", "Pasta", "Soup", "Curry", "Taco", "Pie", "Fritter", "Bowl", "Salad", "Dumpling", "Pickle", "Bread", "Cake"]
...
def _with_categories_edit_option(options: list[str]) -> list[str]:
    cleaned = [str(v).strip() for v in options if str(v).strip() and str(v).strip() != EDIT_IN_CATEGORIES_LABEL]
    return cleaned + [EDIT_IN_CATEGORIES_LABEL]


def _normal_filter_value(value: str, any_label: str) -> str:
    if value in {any_label, EDIT_IN_CATEGORIES_LABEL}:
        return any_label
    return value
...
def render_grocery_starter_packs_tab(sheet_id: str) -> None:
    ...
    st.write("Import optional recipe or specialist libraries into your own Pathmark Sync sheet. Starter packs are still useful for released recipe packs or editable personal copies.")
    st.info("Core shared ingredient reference data, such as seasonality and nutrition, can now be read from the Pathmark Supabase catalogue without copying the whole reference library into each user's Google Sheet. Import packs only when you want editable copies or optional recipe bundles.")
...
def _parse_iso_date_for_widget(value: Any) -> date | None:
    text = str(value or "").strip()
    if not text:
        return None
...
def _recipe_ingredient_text_map(ingredients: pd.DataFrame) -> dict[str, str]:
    mapping: dict[str, list[str]] = {}
    if ingredients is None or ingredients.empty:
        return {}
    for _, row in ingredients.iterrows():
        bits = [
            str(row.get("ingredient", "") or ""),
            str(row.get("notes", "") or ""),
            str(row.get("category_name", "") or ""),
            str(row.get("suggested_substitute", "") or ""),
        ]
        joined = " ".join([b for b in bits if b]).strip()
        for key in [str(row.get("recipe_id", "") or "").strip(), str(row.get("recipe_name", "") or "").strip().lower()]:
            if key:
                mapping.setdefault(key, []).append(joined)
    return {k: " | ".join(v) for k, v in mapping.items()}


def _recipe_search_text(row: pd.Series | dict[str, Any], ingredient_map: dict[str, str] | None = None) -> str:
    getter = row.get if hasattr(row, "get") else (lambda k, d="": "")
    rid = str(getter("recipe_id", "") or "").strip()
    rname = str(getter("recipe_name", "") or "").strip()
    fields = [
        "recipe_name", "category_name", "course", "meal_categories", "cuisine_tags", "dish_style_tags", "dietary_tags",
        "source_title", "author", "protein", "vegetarian_vegan", "gluten_free", "allergens", "months",
        "seasonality_summary", "imported_notes", "frozen_or_canned_notes", "substitute_notes", "homegrown_notes",
        "unavailable_notes", "notes",
    ]
    text_bits = [str(getter(c, "") or "") for c in fields]
    if ingredient_map:
        text_bits.append(ingredient_map.get(rid, ""))
        text_bits.append(ingredient_map.get(rname.lower(), ""))
    return " ".join([b for b in text_bits if b]).lower()


def _field_text(row: pd.Series | dict[str, Any], field: str, ingredient_map: dict[str, str] | None = None) -> str:
    getter = row.get if hasattr(row, "get") else (lambda k, d="": "")
    f = str(field or "").strip().lower()
    if f in {"culture", "cuisine", "style"}:
        return str(getter("cuisine_tags", "") or "").lower()
    if f in {"plate", "dish", "dish_style"}:
        return str(getter("dish_style_tags", "") or "").lower()
    if f in {"diet", "dietary", "requirement", "requirements"}:
        return " ".join([str(getter("dietary_tags", "") or ""), str(getter("vegetarian_vegan", "") or ""), str(getter("gluten_free", "") or "")]).lower()
    if f in {"ingredient", "ingredients", "produce"}:
        rid = str(getter("recipe_id", "") or "").strip()
        rname = str(getter("recipe_name", "") or "").strip().lower()
        return " ".join([(ingredient_map or {}).get(rid, ""), (ingredient_map or {}).get(rname, "")]).lower()
    if f in {"month", "season", "seasonality"}:
        return " ".join([str(getter("months", "") or ""), str(getter("seasonality_summary", "") or "")]).lower()
    if f in {"course"}:
        return str(getter("course", "") or "").lower()
    return str(getter(field, "") or "").lower()


def _recipe_simple_token_match(row: pd.Series | dict[str, Any], token: str, ingredient_map: dict[str, str]) -> bool:
    token = str(token or "").strip().strip('"\'')
    if not token:
        return True
    getter = row.get if hasattr(row, "get") else (lambda k, d="": "")
    low = token.lower()
    m = re.match(r"^time\s*(<=|>=|<|>|=)\s*(\d+(?:\.\d+)?)$", low)
    if m:
        op, num_s = m.group(1), m.group(2)
        try:
            actual = float(str(getter("time_mins", "") or "nan"))
            target = float(num_s)
        except Exception:
            return False
        return {"<=": actual <= target, ">=": actual >= target, "<": actual < target, ">": actual > target, "=": actual == target}.get(op, False)
    m = re.match(r"^(course|culture|cuisine|style|plate|dish|dish_style|diet|dietary|ingredient|ingredients|produce|month|season):(.+)$", token, flags=re.I)
    if m:
        return _contains_loose(_field_text(row, m.group(1), ingredient_map), m.group(2).replace("_", " "))
    return _contains_loose(_recipe_search_text(row, ingredient_map), token.replace("_", " "))


def _split_advanced_or(query: str) -> list[str]:
    return [part.strip() for part in re.split(r"\s+OR\s+", str(query or ""), flags=re.I) if part.strip()]


def _advanced_recipe_query_match(row: pd.Series | dict[str, Any], query: str, ingredient_map: dict[str, str]) -> bool:
    q = str(query or "").strip()
    if not q:
        return True
    # Lightweight Boolean support: OR separates alternatives; AND is optional; NOT negates the next token.
    for alt in _split_advanced_or(q):
        tokens = [t for t in re.split(r"\s+", alt.replace("(", " ").replace(")", " ").strip()) if t and t.upper() != "AND"]
        negate = False
        ok = True
        for token in tokens:
            if token.upper() == "NOT":
                negate = True
                continue
            matched = _recipe_simple_token_match(row, token, ingredient_map)
            if negate:
                matched = not matched
                negate = False
            if not matched:
...
def _shopping_item_title(row: pd.Series | dict[str, Any]) -> str:
    qty = str(row.get("quantity", "") or "").strip()
    unit = str(row.get("unit", "") or "").strip()
    ingredient = str(row.get("ingredient", "") or "").strip()
    return " ".join([x for x in [qty, unit, ingredient] if x]).strip() or ingredient or str(row.get("notes", "") or "").strip()


def _shopping_recipe_source_text(row: pd.Series | dict[str, Any]) -> str:
    recipe_name = str(row.get("recipe_name", "") or "").strip()
    notes = str(row.get("notes", "") or "").strip()
    sources: list[str] = []
    if recipe_name:
        sources.extend([p.strip() for p in re.split(r"[;|]", recipe_name) if p.strip()])
    m = re.search(r"(?:used in|source(?:s)?):\s*(.+)$", notes, flags=re.I)
    if m:
        sources.extend([p.strip() for p in re.split(r"[;|]", m.group(1)) if p.strip()])
    seen = []
    for source in sources:
        if source and source not in seen:
            seen.append(source)
    if len(seen) > 2:
        return f"Used in {len(seen)} recipes: " + "; ".join(seen[:2]) + "..."
    return "; ".join(seen)


def _shopping_merge_key(row: pd.Series | dict[str, Any]) -> tuple[str, str, str, str]:
    ingredient = _normalise_food_key(str(row.get("ingredient", "") or ""))
    category = str(row.get("category_name", "") or "").strip().lower()
    qty = str(row.get("quantity", "") or "").strip().lower()
    unit = str(row.get("unit", "") or "").strip().lower()
    # Exact quantity/unit matches can merge; unquantified repeated items such as salt and pepper also merge.
    return (category, ingredient, qty, unit)


def _combine_source_values(existing: str, incoming: str) -> str:
    values: list[str] = []
...
def _add_trolley_items_to_inventory(sheet_id: str, trolley_items: pd.DataFrame) -> tuple[bool, str]:
    ...
    current_inventory = active_online_df(read_online_table(sheet_id, "grocery_inventory"))
    existing_by_key: dict[str, dict[str, Any]] = {}
    if not current_inventory.empty:
        for _, inv in current_inventory.iterrows():
            key = _normalise_food_key(inv.get("item", ""))
            if key:
    ...
    for _, row in trolley_items.iterrows():
        ...
        if not ingredient:
            continue
        key = _normalise_food_key(ingredient)
        category = str(row.get("category_name", "") or "").strip() or _grocery_category_for_item(ingredient) or "Produce"
        qty = str(row.get("quantity", "") or "").strip()
        unit = str(row.get("unit", "") or "").strip()
        ...
        if existing:
            ...
            existing_by_key[key] = {"inventory_id": inv_id, "item": ingredient}
            new_records.append({
                "inventory_id": inv_id,
                "category_name": category,
                "item": ingredient,
...
PM_MONTH_COLUMNS = ["jan", "feb", "mar", "apr", "may", "jun", "jul", "aug", "sep", "oct", "nov", "dec"]
PM_MONTH_NAMES = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"]


def _ingredient_key(value: Any) -> str:
    return _normalise_food_key(str(value or ""))


def _truthy_reference_value(value: Any) -> bool:
    text = str(value or "").strip().lower()
    return text in {"true", "yes", "y", "1", "season", "in season", "available", "limited"}


def _months_from_reference_row(row: dict[str, Any]) -> str:
    direct = str(row.get("months", "") or row.get("season_months", "") or "").strip()
    if direct:
        return direct
    months = []
    for idx, col in enumerate(PM_MONTH_COLUMNS):
        if _truthy_reference_value(row.get(col, "")):
            months.append(PM_MONTH_NAMES[idx])
    return ", ".join(months)


def _supabase_select_table(table: str, select: str, *, limit: int = 5000, extra: str = "") -> list[dict[str, Any]]:
    if not supabase_available():
        return []
    query = f"?select={urllib.parse.quote(select, safe=',*')}&limit={int(limit)}"
    if extra:
        query += "&" + extra.lstrip("?&")
    ok, payload = supabase_request("GET", table, query=query)
    if ok and isinstance(payload, list):
        return [row for row in payload if isinstance(row, dict)]
    return []


def read_pathmark_ingredient_catalogue(force: bool = False) -> dict[str, Any]:
    """Read shared Pathmark ingredient reference data from Supabase.

    This catalogue is reference data, not user-owned Pathmark Sync data. It is
    ...
    if not force and cache_key in st.session_state:
        ...
        cached = st.session_state.get(cache_key)
        return cached if isinstance(cached, dict) else {"by_key": {}, "records": []}

    if not supabase_available():
        result = {"by_key": {}, "records": [], "available": False, "message": "Supabase is not configured."}
        st.session_state[cache_key] = result
        return result

    record_perf_event("supabase_read", "ingredient catalogue", detail="batched reference lookup")
    ingredients = _supabase_select_table(
        "pathmark_ingredients",
        "ingredient_id,canonical_name,display_name,category_name,default_unit,default_shopping_aisle,notes,status",
        extra="status=eq.active&order=display_name.asc",
    )
    aliases = _supabase_select_table(
        "pathmark_ingredient_aliases",
        "ingredient_id,alias,status",
        extra="status=eq.active",
    )
    seasonality = _supabase_select_table(
        "pathmark_produce_seasonality",
        "ingredient_id,region,months,season_months,jan,feb,mar,apr,may,jun,jul,aug,sep,oct,nov,dec,season_note,status",
        extra="status=eq.active",
    )
    nutrition = _supabase_select_table(
        "pathmark_nutrition_reference",
    ...
    for row in seasonality:
        ...
        if iid and iid not in season_by_id:
            season_by_id[iid] = row
    nutrition_by_id: dict[str, dict[str, Any]] = {}
    ...
    for row in ingredients:
        iid = str(row.get("ingredient_id", "") or "").strip()
        display = str(row.get("display_name", "") or row.get("canonical_name", "") or "").strip()
        canonical = str(row.get("canonical_name", "") or display).strip()
        season = season_by_id.get(iid, {})
        nutr = nutrition_by_id.get(iid, {})
        rec = {
            "ingredient_id": iid,
            ...
            "default_shopping_aisle": str(row.get("default_shopping_aisle", "") or ""),
            "months": _months_from_reference_row(season),
            "seasonality_notes": str(season.get("season_note", "") or ""),
            "kcal_per_100g": str(nutr.get("kcal", "") or ""),
            "protein": str(nutr.get("protein", "") or ""),
            "carbohydrate": str(nutr.get("carbohydrate", "") or ""),
            "fat": str(nutr.get("fat", "") or ""),
            "fibre": str(nutr.get("fibre", "") or ""),
            "sodium": str(nutr.get("sodium", "") or ""),
            "nutrition_notes": str(nutr.get("notes", "") or ""),
            "notes": str(row.get("notes", "") or ""),
            "source": "Pathmark data",
        }
        records.append(rec)
        if iid:
            by_id[iid] = rec
...
def read_user_ingredient_overrides(sheet_id: str) -> pd.DataFrame:
    if not sheet_id or "ingredient_overrides" not in ONLINE_TABLES:
        return pd.DataFrame(columns=ONLINE_TABLES.get("ingredient_overrides", []))
    return active_online_df(read_online_table(sheet_id, "ingredient_overrides"))


def _override_lookup(overrides: pd.DataFrame) -> dict[str, dict[str, Any]]:
    lookup: dict[str, dict[str, Any]] = {}
    if overrides.empty:
        return lookup
    for _, row in overrides.iterrows():
        row_dict = row.to_dict()
        keys = [row_dict.get("ingredient_key", ""), row_dict.get("ingredient", ""), row_dict.get("display_name", "")]
...
def resolve_ingredient_reference(ingredient: Any, catalogue: dict[str, Any] | None = None, overrides: pd.DataFrame | None = None) -> dict[str, Any]:
    text = str(ingredient or "").strip()
    key = _ingredient_key(text)
    catalogue = catalogue or read_pathmark_ingredient_catalogue()
    user_lookup = _override_lookup(overrides if overrides is not None else pd.DataFrame())
    if key and key in user_lookup:
        row = user_lookup[key]
        return {
            "source": "Your data",
            "source_detail": "User override",
            "ingredient_key": key,
            "ingredient_id": str(row.get("pathmark_ingredient_id", "") or ""),
            "display_name": str(row.get("display_name", "") or row.get("ingredient", "") or text),
            "preferred_unit": str(row.get("preferred_unit", "") or ""),
            "preferred_aisle": str(row.get("preferred_aisle", "") or ""),
            "months": str(row.get("months", "") or ""),
            "seasonality_notes": str(row.get("seasonality_notes", "") or ""),
            "kcal_per_100g": str(row.get("kcal_per_100g", "") or ""),
            "protein": str(row.get("protein", "") or ""),
            "carbohydrate": str(row.get("carbohydrate", "") or ""),
            "fat": str(row.get("fat", "") or ""),
            "fibre": str(row.get("fibre", "") or ""),
            "sodium": str(row.get("sodium", "") or ""),
            "nutrition_notes": str(row.get("nutrition_notes", "") or ""),
            "notes": str(row.get("notes", "") or ""),
            "matched": True,
            "custom": True,
        }
    by_key = catalogue.get("by_key", {}) if isinstance(catalogue, dict) else {}
    rec = by_key.get(key) if key else None
    if rec:
        return {
            "source": "Pathmark data",
            "source_detail": "Shared catalogue",
            "ingredient_key": key,
            "ingredient_id": str(rec.get("ingredient_id", "") or ""),
            "display_name": str(rec.get("display_name", "") or text),
            "preferred_unit": str(rec.get("default_unit", "") or ""),
            "preferred_aisle": str(rec.get("default_shopping_aisle", "") or rec.get("category_name", "") or ""),
            "months": str(rec.get("months", "") or ""),
            "seasonality_notes": str(rec.get("seasonality_notes", "") or ""),
            "kcal_per_100g": str(rec.get("kcal_per_100g", "") or ""),
            "protein": str(rec.get("protein", "") or ""),
            "carbohydrate": str(rec.get("carbohydrate", "") or ""),
            "fat": str(rec.get("fat", "") or ""),
            "fibre": str(rec.get("fibre", "") or ""),
            "sodium": str(rec.get("sodium", "") or ""),
            "nutrition_notes": str(rec.get("nutrition_notes", "") or ""),
            "notes": str(rec.get("notes", "") or ""),
            "matched": True,
            "custom": False,
        }
    return {
        "source": "Not matched",
        "source_detail": "No Pathmark catalogue match",
        "ingredient_key": key,
        "ingredient_id": "",
        "display_name": text,
        "preferred_unit": "",
...
def _ingredient_reference_rows(sheet_id: str, grouped: pd.DataFrame) -> pd.DataFrame:
    catalogue = read_pathmark_ingredient_catalogue()
    overrides = read_user_ingredient_overrides(sheet_id)
    rows: list[dict[str, Any]] = []
...
def _save_ingredient_override(sheet_id: str, base: dict[str, Any], fields: dict[str, Any]) -> tuple[bool, str]:
    ingredient = str(base.get("Ingredient", "") or fields.get("ingredient", "") or "").strip()
    if not ingredient:
        return False, "Choose an ingredient first."
    record = {
        "override_id": f"ingredient-override-{uuid.uuid4().hex[:12]}",
        "ingredient_key": _ingredient_key(ingredient),
        "ingredient": ingredient,
        "pathmark_ingredient_id": str(base.get("pathmark_ingredient_id", "") or fields.get("pathmark_ingredient_id", "") or ""),
        "display_name": str(fields.get("display_name", "") or ingredient).strip(),
        "preferred_unit": str(fields.get("preferred_unit", "") or "").strip(),
        "preferred_aisle": str(fields.get("preferred_aisle", "") or "").strip(),
        "months": str(fields.get("months", "") or "").strip(),
        "seasonality_notes": str(fields.get("seasonality_notes", "") or "").strip(),
        "kcal_per_100g": str(fields.get("kcal_per_100g", "") or "").strip(),
        "protein": str(fields.get("protein", "") or "").strip(),
        "carbohydrate": str(fields.get("carbohydrate", "") or "").strip(),
        "fat": str(fields.get("fat", "") or "").strip(),
        "fibre": str(fields.get("fibre", "") or "").strip(),
        "sodium": str(fields.get("sodium", "") or "").strip(),
        "nutrition_notes": str(fields.get("nutrition_notes", "") or "").strip(),
        "override_seasonality": "TRUE" if fields.get("override_seasonality") else "",
        "override_nutrition": "TRUE" if fields.get("override_nutrition") else "",
        "notes": str(fields.get("notes", "") or "").strip(),
        "status": "active",
        "source": "User ingredient override",
    }
    return append_online_record(sheet_id, "ingredient_overrides", record)

def render_ingredients_validation_tab(sheet_id: str) -> None:
    st.subheader("Ingredients")
    st.caption("Ingredient names and units used by recipes, shopping and inventory. Pathmark uses shared reference data when available, and only saves your own overrides to Pathmark Sync.")

    c_info, c_refresh = st.columns([3, 1])
    with c_info:
        st.info("Seasonality and nutrition can come from the shared Pathmark catalogue in Supabase. Those reference rows are not copied into your Google Sheet unless you customise or add your own data.")
    with c_refresh:
        if st.button("Refresh catalogue", use_container_width=True, key="refresh_ingredient_catalogue"):
            read_pathmark_ingredient_catalogue(force=True)
            st.rerun()
    ...
    df = pd.DataFrame(rows)
    if df.empty:
        st.info("No ingredient validation rows yet. Add recipe ingredients, add stock, or import recipes.")
        grouped = pd.DataFrame(columns=["ingredient", "unit", "category_name", "source"])
    else:
        df["ingredient_key"] = df["ingredient"].map(_ingredient_key)
    ...
    reference_view = _ingredient_reference_rows(sheet_id, grouped) if not grouped.empty else pd.DataFrame(columns=["Ingredient", "Units used", "Shopping aisle", "Data source", "Matched name", "Seasonality", "Nutrition", "Used by", "ingredient_key", "pathmark_ingredient_id"])
    search = st.text_input("Search ingredients", key="ingredient_validation_search", placeholder="e.g., cavolo nero, tofu, barley")
    source_filter = st.selectbox("Data source", ["All sources", "Pathmark data", "Your data", "Not matched"], key="ingredient_source_filter")
    view = reference_view.copy()
    if search.strip() and not view.empty:
        term = search.strip().lower()
        view = view[view.apply(lambda r: term in " ".join(str(v).lower() for v in r.values), axis=1)]
    ...
    if not reference_view.empty:
        summary = reference_view["Data source"].value_counts().to_dict()
        bits = [f"{label}: {count}" for label, count in summary.items()]
        st.caption("Ingredient data sources — " + " · ".join(bits))

    with st.expander("Customise or add ingredient data", expanded=False):
        st.caption("Use this when an ingredient is not in the Pathmark catalogue, or when you want your own unit, aisle, seasonality or nutrition values. Pathmark saves only this override to your Google Sheet.")
        options = [str(x).strip() for x in reference_view.get("Ingredient", pd.Series(dtype=str)).tolist() if str(x).strip()]
        selected = st.selectbox("Ingredient", options if options else [""], key="ingredient_override_select")
        selected_row = reference_view[reference_view.get("Ingredient", pd.Series(dtype=str)).fillna("").astype(str).eq(selected)].iloc[0].to_dict() if selected and not reference_view.empty and selected in set(reference_view.get("Ingredient", [])) else {"Ingredient": selected}
        catalogue = read_pathmark_ingredient_catalogue()
        overrides = read_user_ingredient_overrides(sheet_id)
        current_ref = resolve_ingredient_reference(selected, catalogue, overrides) if selected else {}
        c1, c2, c3 = st.columns(3)
        with c1:
            display_name = st.text_input("Display name", value=str(current_ref.get("display_name", selected) or selected), key="ingredient_override_display")
            preferred_unit = st.text_input("Preferred unit", value=str(current_ref.get("preferred_unit", "") or ""), key="ingredient_override_unit")
        with c2:
            preferred_aisle = st.text_input("Preferred shopping aisle", value=str(current_ref.get("preferred_aisle", "") or selected_row.get("Shopping aisle", "") or ""), key="ingredient_override_aisle")
            months = st.text_input("Months / seasonality", value=str(current_ref.get("months", "") or ""), placeholder="e.g., June, July, August", key="ingredient_override_months")
        with c3:
            kcal_per_100g = st.text_input("kcal per 100 g", value=str(current_ref.get("kcal_per_100g", "") or ""), key="ingredient_override_kcal")
            protein = st.text_input("Protein", value=str(current_ref.get("protein", "") or ""), key="ingredient_override_protein")
        c4, c5, c6 = st.columns(3)
        with c4:
            carbohydrate = st.text_input("Carbohydrate", value=str(current_ref.get("carbohydrate", "") or ""), key="ingredient_override_carbs")
        with c5:
            fat = st.text_input("Fat", value=str(current_ref.get("fat", "") or ""), key="ingredient_override_fat")
        with c6:
            fibre = st.text_input("Fibre", value=str(current_ref.get("fibre", "") or ""), key="ingredient_override_fibre")
        sodium = st.text_input("Sodium", value=str(current_ref.get("sodium", "") or ""), key="ingredient_override_sodium")
        seasonality_notes = st.text_area("Seasonality notes", value=str(current_ref.get("seasonality_notes", "") or ""), key="ingredient_override_season_notes")
        nutrition_notes = st.text_area("Nutrition notes", value=str(current_ref.get("nutrition_notes", "") or ""), key="ingredient_override_nutrition_notes")
        notes = st.text_area("Personal notes", value="", key="ingredient_override_notes")
        o1, o2 = st.columns(2)
        with o1:
            override_seasonality = st.checkbox("Use my seasonality instead of Pathmark data", value=bool(str(current_ref.get("source", "")) != "Pathmark data"), key="ingredient_override_seasonality_flag")
        with o2:
            override_nutrition = st.checkbox("Use my nutrition instead of Pathmark data", value=bool(str(current_ref.get("source", "")) != "Pathmark data"), key="ingredient_override_nutrition_flag")
        if st.button("Save ingredient data", use_container_width=True, disabled=not bool(selected), key="save_ingredient_override"):
            fields = {
                ...
                "notes": notes,
                "override_seasonality": override_seasonality,
                "override_nutrition": override_nutrition,
                "pathmark_ingredient_id": str(selected_row.get("pathmark_ingredient_id", "") or current_ref.get("ingredient_id", "") or ""),
            }
            ok, msg = _save_ingredient_override(sheet_id, selected_row, fields)
            if ok:
                st.success("Ingredient data saved to your Pathmark Sync sheet.")
                st.rerun()
            else:
                st.warning(safe_user_message(msg))
...
def render_shopping_items_as_planner(sheet_id: str, list_id: str, selected_list: str, items: pd.DataFrame) -> None:
    ...
    css = """
    <style>
      .pm-shop-category { margin-top: 1rem; margin-bottom: 0.25rem; font-weight: 760; letter-spacing: -0.01em; }
...
def render_grocery_categories_tab(sheet_id: str) -> None:
    ...
    st.markdown("#### Shopping aisles")
    st.caption("These are supermarket aisle/category groups for shopping lists. They stay separate from recipe discovery filters.")
    ...
    with st.form("add_grocery_category_form"):
        st.markdown("#### Add shopping aisle/category")
    ...
    st.caption("These are customisable recipe discovery filters. Shopping aisles stay separate from recipe filters.")
    recipe_tables = [
        ("Courses", "recipe_courses", "course_id", "course_name", "course"),
        ("Culture / style", "recipe_cuisine_tags", "cuisine_tag_id", "cuisine_tag_name", "cuisine_tag"),
        ("Plate / dish style", "recipe_dish_style_tags", "dish_style_tag_id", "dish_style_tag_name", "dish_style_tag"),
        ("Dietary requirements", "recipe_dietary_tags", "dietary_tag_id", "dietary_tag_name", "dietary_tag"),
    ]
    for label, table, id_col, name_col, prefix in recipe_tables:
        with st.expander(label, expanded=False):
            ...
            with st.form(f"add_{table}_form"):
                ...
                add_tag = st.form_submit_button("Add option", use_container_width=True)
            if add_tag:
...
def _recipe_filter_mask(
    ...
    *,
    ...
    if cultures:
        wanted = [c.lower() for c in cultures if c]
        mask &= recipes["cuisine_tags"].fillna("").astype(str).str.lower().map(lambda text: any(w in text for w in wanted))
    if plates:
        wanted = [p.lower() for p in plates if p]
        mask &= recipes["dish_style_tags"].fillna("").astype(str).str.lower().map(lambda text: any(w in text for w in wanted))
    if dietary:
        wanted = [d.lower() for d in dietary if d]
        mask &= recipes.apply(lambda r: any(w in _field_text(r, "dietary", ingredient_map) for w in wanted), axis=1)
    if time_limit and time_limit != "Any time":
        limit_map = {"15 minutes or less": 15, "30 minutes or less": 30, "45 minutes or less": 45, "1 hour or less": 60, "Over 1 hour": None}
        numeric_time = pd.to_numeric(recipes.get("time_mins", pd.Series([None] * len(recipes), index=recipes.index)), errors="coerce")
        if time_limit == "Over 1 hour":
            mask &= numeric_time.gt(60)
        elif time_limit in limit_map and limit_map[time_limit] is not None:
            mask &= numeric_time.le(limit_map[time_limit])
    if vegetarian_only:
        mask &= recipes.apply(lambda r: "vegetarian" in _field_text(r, "dietary", ingredient_map) or str(r.get("vegetarian", "") or "").strip().lower() in {"yes", "true", "1"}, axis=1)
    if vegan_only:
        mask &= recipes.apply(lambda r: "vegan" in _field_text(r, "dietary", ingredient_map) or str(r.get("vegan", "") or "").strip().lower() in {"yes", "true", "1"}, axis=1)
    if gluten_free_only:
        mask &= recipes.apply(lambda r: "gluten free" in _field_text(r, "dietary", ingredient_map) or "gluten" in str(r.get("gluten_free", "") or "").lower(), axis=1)
    ...
    include = _split_search_terms(include_terms)
    exclude = _split_search_terms(exclude_terms)
    if include:
        mask &= recipes.apply(lambda r: all(_contains_loose(_recipe_search_text(r, ingredient_map), term) for term in include), axis=1)
    if exclude:
        mask &= ~recipes.apply(lambda r: any(_contains_loose(_recipe_search_text(r, ingredient_map), term) for term in exclude), axis=1)
    if str(advanced_query or "").strip():
        mask &= recipes.apply(lambda r: _advanced_recipe_query_match(r, advanced_query, ingredient_map), axis=1)
    return mask
...
def render_recipe_library_tab(sheet_id: str) -> None:
    ...
    with st.expander("Search recipe library", expanded=False):
        ...
        with c1:
            ...
            filter_cultures_raw = st.multiselect("Culture / style", _with_categories_edit_option(cuisine_options), key="recipe_filter_cultures")
            filter_plates_raw = st.multiselect("Plate / dish style", _with_categories_edit_option(dish_style_options), key="recipe_filter_plates")
        with c2:
            filter_time = st.selectbox("Time", time_filter_options, key="recipe_filter_time")
            filter_time_sort = st.selectbox("Time sort", time_sort_options, key="recipe_filter_time_sort")
            filter_months = st.multiselect("Month / season", GROCERY_MONTHS, key="recipe_filter_months")
        with c3:
        ...
        with st.expander("Advanced search", expanded=False):
            advanced_query = st.text_input("Boolean-style search", placeholder="japanese AND vegetarian NOT mushroom", key="recipe_advanced_query")
            st.caption("Supports AND, OR, NOT and simple fields such as course:dinner, culture:japanese, plate:nimono, ingredient:tofu, month:june, time<=45.")
        edit_hint_needed = EDIT_IN_CATEGORIES_LABEL in set(filter_courses_raw + filter_cultures_raw + filter_plates_raw + filter_dietary_raw)
        if edit_hint_needed:
            st.info("These options can be edited in Nutrition → Categories. This selection is a shortcut, not a filter.")
        filter_courses = [v for v in filter_courses_raw if v != EDIT_IN_CATEGORIES_LABEL]
        filter_cultures = [v for v in filter_cultures_raw if v != EDIT_IN_CATEGORIES_LABEL]
        ...
        filter_dietary = [v for v in filter_dietary_raw if v != EDIT_IN_CATEGORIES_LABEL]
        active_chips = []
        for label, values in [("Course", filter_courses), ("Culture", filter_cultures), ("Plate", filter_plates), ("Dietary", filter_dietary), ("Month", filter_months)]:
            active_chips.extend([f"{label}: {v}" for v in values])
        if filter_time != "Any time":
            active_chips.append(f"Time: {filter_time}")
        if include_terms.strip():
            active_chips.append(f"Include: {include_terms.strip()}")
        if exclude_terms.strip():
            active_chips.append(f"Exclude: {exclude_terms.strip()}")
        if advanced_query.strip():
            active_chips.append(f"Advanced: {advanced_query.strip()}")
        if active_chips:
            st.caption("Active filters: " + " · ".join(active_chips))
    filtered = recipes.copy()
    ...
    if filtered.empty:
        ...
        show_cols = [c for c in ["recipe_name", "course", "cuisine_tags", "dish_style_tags", "dietary_tags", "servings", "time_mins", "months", "source_title", "page"] if c in filtered.columns]
    ...
    with st.form("add_recipe_form"):
        ...
        with c1:
            ...
            recipe_category = st.selectbox("Primary shopping aisle/category", categories, key="recipe_category")
        ...
        culture_tags = st.multiselect("Culture / style", cuisine_options, key="recipe_culture_tags")
        dish_tags = st.multiselect("Plate / dish style", dish_style_options, key="recipe_dish_tags")
        dietary_tags = st.multiselect("Dietary requirements", dietary_options, key="recipe_dietary_tags")
    ...
    if submitted:
        if not recipe_name.strip():
            ...
            record = {
                ...
                "course": course,
                "meal_categories": "",
                "cuisine_tags": _join_tags(culture_tags),
...
def render_shopping_lists_tab(sheet_id: str) -> None:
    ...
    st.caption("Create supermarket-aisle shopping lists from recipes or individual ingredients. Bought items move to In trolley and can be brought back with one click.")
    categories = grocery_category_options(sheet_id)
    lists = active_online_df(read_online_table(sheet_id, "shopping_lists"))
    items = active_online_df(read_online_table(sheet_id, "shopping_items"))
    ...
    # Expiry dates and pantry reminders are handled from the Inventory tab after shopping.
