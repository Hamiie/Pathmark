...
APP_TITLE = "Development System Demo"
APP_VERSION = "demo v0.1"
ROOT = Path(__file__).resolve().parents[1]
DATA_DIR = ROOT / "data"
DB_PATH = DATA_DIR / "development_system.db"
OUTPUT_DIR = ROOT / "output"
CONFIG_DIR = ROOT / "config"
DEMO_MODE = True
DEFAULT_DEVELOPMENT_ROOT = ROOT / "demo_workspace"
DEFAULT_SYSTEM_FOLDER = DEFAULT_DEVELOPMENT_ROOT / "00_System"
DEFAULT_CALENDAR_EXPORT_FOLDER = DEFAULT_DEVELOPMENT_ROOT / "CalendarExports"
DEFAULT_GOOGLE_TASKS_EXPORT_FOLDER = DEFAULT_DEVELOPMENT_ROOT / "GoogleTasksExports"
...
def seasonal_theme(mode: str) -> dict[str, str]:
    ...
    return dict(palettes.get(season, palettes["Autumn"]).get("dark" if mode == "Dark" else "light"))


def theme() -> dict[str, str]:
    config = load_yaml(CONFIG_DIR / "theme.yaml")
    mode = get_setting("theme_mode", "Dark")
    preset = get_setting("theme_preset", "Seasonal")
    if preset == "Seasonal":
        return seasonal_theme(mode)
    presets = config.get("themes", {})
    if preset == "Custom":
        return custom_theme(mode)
    selected = presets.get(preset) or presets.get("Default") or {}
    return selected.get("dark" if mode == "Dark" else "light", {})

def custom_theme(mode: str) -> dict[str, str]:
    base = load_yaml(CONFIG_DIR / "theme.yaml").get("themes", {}).get("Default", {})
    ...
    with connect() as con:
        con.executescript(
            ...
            CREATE TABLE IF NOT EXISTS reminder_tasks (
                ...
                due_date TEXT,
                reminder_time TEXT,
                task_list TEXT DEFAULT 'Development System',
                export_status TEXT DEFAULT 'Staged',
...
def backup_database(reason: str = "manual") -> Path | None:
    if not DB_PATH.exists():
        return None
    backup_dir = OUTPUT_DIR / "backups" / "database"
    backup_dir.mkdir(parents=True, exist_ok=True)
    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    backup = backup_dir / f"development_system_{reason}_{stamp}.db"
...
def ensure_default_settings() -> None:
    defaults = {
        "theme_mode": "Dark",
        "theme_preset": "Seasonal",
        "weekly_focus": "No main focus",
        "system_name": "Development System Demo",
        ...
        "default_task_list": "Development System Demo",
        "default_calendar": "Development System Demo",
        "setup_complete": "Yes",
...
def seed_data() -> None:
    ...
    areas = [
        ("Body And Stability", "Movement, sleep, food, recovery, and sustainable capacity", "01_Body_And_Stability", "README", "Body And Stability", "Development System", "Sage"),
        ("Home And Garden", "Household reset, garden, houseplants, and domestic environment", "02_Home_And_Garden", "README", "Home And Garden", "Development System", "Basil"),
        ("Making And Craft", "Pottery, design, glaze, clay, and making projects", "03_Making_And_Craft", "README", "Making And Craft", "Development System", "Tangerine"),
        ("Expression And Culture", "Language, music, art practice, reading, and cultural work", "04_Expression_And_Culture", "README", "Expression And Culture", "Development System", "Peacock"),
        ("Work And Admin", "Professional work, planning, administration, and system maintenance", "05_Work_And_Admin", "README", "Work And Admin", "Development System", "Graphite"),
        ("Finance", "Budgeting, forecasting, savings, and financial administration", "06_Finance", "README", "Finance", "Development System", "Grape"),
    ]
...
def area_options(include_all: bool = False) -> list[str]:
    data = areas_df(active_only=False)
...
def stage_exports_from_actions(area_id: int | None = None, routine_task_occurrences: int = 1) -> None:
    ...
    for action in query(f"SELECT * FROM actions {where}", params):
        ...
        task_list = area[0]["default_task_list"] if area else "Development System"
...
def create_development_root() -> None:
    ...
    # the configured Development root folder only. pathlib.mkdir(..., exist_ok=True)
...
def ensure_core_folders() -> None:
    ...
    for folder in [root_folder(), system_folder(), calendar_export_folder(), google_tasks_export_folder(), OUTPUT_DIR / "weekly", OUTPUT_DIR / "backups"]:
...
def backup_file_if_exists(path: Path) -> None:
    if not path.exists():
        return
    backup_dir = OUTPUT_DIR / "backups" / "markdown"
    backup_dir.mkdir(parents=True, exist_ok=True)
    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
...
def existing_numbered_area_folders() -> list[tuple[int, Path]]:
    """Return numbered area folders already present under the Development root.

    This is deliberately read-only. It only inspects folder names such as
    01_Body_And_Stability and never deletes, renames, or empties anything.
    """
    root = root_folder()
    if not root.exists():
        return []
...
def suggested_area_folder(area_name: str) -> Path:
    ...
    numeric sequence under the Development root, e.g. 07_New_Area.
    """
...
def export_ics(rows: list[dict[str, Any]], output_path: Path) -> Path:
    ...
    lines = ["BEGIN:VCALENDAR", "VERSION:2.0", "PRODID:-//Development System//Google Calendar Export//EN", "CALSCALE:GREGORIAN"]
...
def export_tasks_csv(rows: list[dict[str, Any]], output_path: Path) -> Path:
    ...
    with output_path.open("w", newline="", encoding="utf-8-sig") as f:
        ...
        for r in rows:
            ...
            writer.writerow({
                "Task ID": source_id,
                "Task List": r.get("task_list") or "Development System",
...
def home_page() -> None:
    ...
    if st.button("Run review checks"):
        generate_review_issues()
...
def this_week_page() -> None:
    ...
    if st.button("Create tasklist PDF"):
        if not selected_action_ids and not fixed_notes.strip():
            ...
            path = weekly_pdf(OUTPUT_DIR / "weekly" / f"tasklist_{date.today().strftime('%Y%m%d')}.pdf", selected_action_ids, [], fixed_notes, title=title)
...
def goal_form(goal: dict[str, Any] | None = None) -> None:
    ...
    with st.form(f"goal_form_{goal['id'] if goal else 'new'}"):
        ...
        area_name = st.selectbox("Area", area_names, index=default_area if area_names else None, help="The main Development folder area, such as Body And Stability or Work And Admin.")
...
def areas_page() -> None:
    page_title("Areas", "Match the app to your Development folder system and generate Markdown files.")
    help_note(f"Folders are created under {root_folder()} if missing. Existing files and non-empty folders are not deleted or emptied. Markdown files may be updated, with backups created first. New area folders continue the existing numbered sequence, for example 07_New_Area.")
    if st.button("Create missing area folders and Markdown files"):
        regenerate_area_markdown()
        st.success("Development folders and Markdown files updated.")
...
def calendar_export_page() -> None:
    page_title("Google Calendar Export", "Review the visible Google Calendar time blocks, then export them as an ICS file for Google Calendar.")
    area = st.selectbox("Area", area_options(include_all=True), key="cal_area_filter")
    area_id = area_id_by_name(area)
    if st.button("Prepare export rows"):
        stage_exports_from_actions(area_id=area_id)
        st.success("Google Calendar exports prepared.")
    ...
    st.markdown("<div class='small-muted'>Visible rows will be exported. Select rows in the table only if you want to remove them from the export list. Exports are written to the configured GoogleTasksExports folder.</div>", unsafe_allow_html=True)
    c1,c2 = st.columns(2)
    if c1.button("Export visible rows as ICS"):
...
def google_tasks_export_page() -> None:
    page_title("Google Tasks Export", "Review the visible starting prompts and export them as a CSV file using your Google Tasks import template. Repeating routines export one next prompt by default; reference times are written into notes because Google Tasks due dates do not store a time.")
    ...
    if st.button("Prepare export rows"):
        ...
        st.success("Google Tasks exports prepared.")
    ...
    st.markdown("<div class='small-muted'>Visible rows will be exported. Select rows in the table only if you want to remove them from the export list. Exports are written to the configured GoogleTasksExports folder.</div>", unsafe_allow_html=True)
    c1,c2 = st.columns(2)
    if c1.button("Export visible rows as CSV"):
        rows = data.to_dict("records")
        suffix = slug(area if area != "All areas" else "visible")
...
def archive_page() -> None:
    ...
    if c3.button("Back up and clear archive"):
        backup = OUTPUT_DIR / "backups" / f"archive_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
...
def setup_page() -> None:
    apply_css()
    page_title("Set up the demo", "Choose where this demo should store temporary folders, Markdown, exports, and starter data.")
    st.write("This deployment package is demo-safe. It writes inside the app workspace, creates missing folders only, and is not connected to any personal OneDrive folder.")
    with st.form("first_run_setup"):
        system_name = st.text_input("System name", value=get_setting("system_name", "Development System") or "Development System")
        root = st.text_input("Root folder", value=get_setting("root_folder", str(DEFAULT_DEVELOPMENT_ROOT)) or str(DEFAULT_DEVELOPMENT_ROOT), help="Main Development folder. Existing files are left in place.")
        mode = st.radio("Mode", ["Light", "Dark"], index=1, horizontal=True)
        preset = st.selectbox("Theme", ["Seasonal", "Default", "Mid-century", "Scandi", "Japandi", "Gamer", "Custom"], index=0)
        default_calendar = st.text_input("Default Google Calendar", value=get_setting("default_calendar", "Development System"))
        default_task_list = st.text_input("Default Google Tasks list", value=get_setting("default_task_list", "Development System"))
        load_examples = st.checkbox("Load editable starter examples", value=True)
...
def settings_page() -> None:
    ...
    with setup_tab:
        ...
        with st.form("settings_setup_form"):
            system_name = st.text_input("System name", value=get_setting("system_name", "Development System"))
            root = st.text_input(
                "Root folder",
                value=str(root_folder()),
                help="Main Development folder. The app creates missing area folders and Markdown under this path without deleting existing files.",
            )
        ...
        if submitted:
            ...
            set_setting("default_task_list", default_task_list)
            ensure_core_folders()
            st.success("Settings saved. Missing folders were created if needed; existing files were not deleted or emptied.")
            st.rerun()

        st.markdown("---")
        st.subheader("Folder actions")
        st.caption(f"Current root folder: `{root_folder()}`")
        col_a, col_b, col_c = st.columns(3)
        with col_a:
            if st.button("Open Development folder", use_container_width=True):
                open_folder(root_folder())
        with col_b:
            if st.button("Create missing folders", use_container_width=True):
                ensure_core_folders()
                st.success("Development folders are ready. Existing files were left in place.")
        with col_c:
            if st.button("Create area folders and indexes", use_container_width=True):
                regenerate_area_markdown()
        ...
        with col_a:
            if st.button("Create goal and routine Markdown", use_container_width=True):
                regenerate_goal_and_routine_markdown()
                st.success("Goal and routine Markdown files updated.")
        with col_b:
            if st.button("Create all Markdown", use_container_width=True):
                regenerate_area_markdown()
                regenerate_goal_and_routine_markdown()
    ...
    with appearance_tab:
        ...
        current_mode = get_setting("theme_mode", "Dark")
        current_preset = get_setting("theme_preset", "Seasonal")
        mode = st.radio(
            "Mode",
            ["Light", "Dark"],
            horizontal=True,
            index=["Light", "Dark"].index(current_mode) if current_mode in ["Light", "Dark"] else 1,
            key="theme_mode_select",
        )
        preset_options = ["Seasonal", "Default", "Mid-century", "Scandi", "Japandi", "Gamer", "Custom"]
        preset = st.selectbox(
            "Theme",
            preset_options,
            index=preset_options.index(current_preset) if current_preset in preset_options else 0,
            key="theme_preset_select",
            help="Seasonal follows the current seasonal palette. Custom lets you test your own palette.",
        )
    ...
    with maintenance_tab:
        ...
        col_a, col_b = st.columns(2)
        with col_a:
            if st.button("Create missing standard routines", use_container_width=True):
                created = ensure_standard_routines()
                st.success(f"Created {created} missing standard routine(s).")
        with col_b:
            if st.button("Run review checks", use_container_width=True):
                generate_review_issues()
        ...
        with col_b:
            uploaded_db = st.file_uploader(
                ...
                help="Restoring replaces the local app database after creating a backup.",
            )
    ...
    with compatibility_tab:
        ...
        col_a, col_b, col_c = st.columns(3)
        with col_a:
            if st.button("Create sample ICS", use_container_width=True):
                ...
                st.success(f"Created {path.name}")
        with col_b:
            if st.button("Create sample Google Tasks CSV", use_container_width=True):
                sample = [{"id":"DIAG001","task_list":get_setting("default_task_list", "Development System"),"title":"Open the diagnostic task","notes":"Sample file for Google Tasks Apps Script import testing.","due_date":date.today().isoformat(),"reminder_time":"09:00","status":"needsAction"}]
...
def main() -> None:
    ...
    if get_setting("load_sample_data", "Yes") == "Yes" and table_count("areas") == 0:
        seed_data()
    apply_css()
    st.markdown("<div class='demo-banner'>Demo mode — use sample or non-sensitive data only. Exports are generated as downloadable files and hosted data may reset.</div>", unsafe_allow_html=True)
    st.sidebar.title(get_setting("system_name", "Development System"))
    st.sidebar.caption(f"Version {APP_VERSION} — shareable demo")
    st.sidebar.info("Demo mode: do not enter private information. Hosted data may reset when the app restarts.")
    pages = {
        "Home": home_page,
