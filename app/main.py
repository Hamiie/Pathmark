...
def package_path(version: dict) -> Path:
    name = version.get("package_name") or "Pathmark_Local_App.zip"
...
<div class="hero-wrap">
  ...
  <div class="product-panel">
    <div class="panel-top">
      ...
      <p>Pathmark runs in your browser, but the real app lives on your computer so it can write Markdown, backups, tasklists, and Google export files where you choose.</p>
      <div class="mini-list">
...
st.header("What Pathmark helps with")
...
with left:
    st.write(
        "Pathmark separates replaceable app files from your lasting system files. "
        "When you update, you replace the app. You do not replace your Pathmark folder, database, Markdown, backups, or exports."
    )
    st.markdown(
        """
<div class="safe-rule"><strong>The app folder is replaceable.</strong><br>The Pathmark folder is not.</div>
""",
        unsafe_allow_html=True,
    )
with right:
...
with new_tab:
    st.subheader("Install Pathmark locally")
...
with learn_tab:
    st.subheader("Why Pathmark runs locally")
    st.write(
        "The hosted page is a polished front door, but a cloud app cannot write to your Documents folder or OneDrive. "
        "Pathmark's main benefit is local file integration, so the real app runs locally and opens in the browser at localhost."
    )
...
- Markdown generation into the chosen Pathmark folder.
...
with s1:
    st.markdown(
        """
<div class="feature-card"><h3>Pathmark may</h3><p>Create missing folders, generate Markdown, make backups, and write exports into <code>00_System</code>.</p></div>
""",
        unsafe_allow_html=True,
    )
with s2:
    st.markdown(
        """
<div class="feature-card"><h3>Pathmark should not</h3><p>Delete files, empty folders, replace your Pathmark folder during updates, or overwrite Markdown without a backup.</p></div>
""",
        unsafe_allow_html=True,
    )
...
st.caption("Pathmark release hub. Local files stay on the user's computer.")
